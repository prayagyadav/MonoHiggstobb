...
def single_tight_muons(events, cutflow): 
...
def HEM_veto_bril(events, cutflow , cat="resolved"):
    """
    ...
    if is_data == True :
        ...
        if cat == "resolved":
            ...
            raise KeyError("Boosted not yet completed.")
...
def get_recoil(events, leptons):
    ...
    vec1 = ak.zip( {"x": flat_single_leptons.pt*np.cos(flat_single_leptons.phi), "y": flat_single_leptons.pt*np.sin(flat_single_leptons.phi), }, with_name="TwoVector", behavior=vector.behavior,)
    ...
    #recoil is a numpy array(or a single level awkward array) with the same number of elements as events and leptons
    return recoil
